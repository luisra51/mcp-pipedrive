// Package mcppipedrive provides generic MCP tool helpers: the reflection-based
// schema generator and the MustTool/ConvertTool factory that wires Go handler
// funcs into the mcp-go server. Lives at the module root because Go package
// names cannot contain hyphens, so "mcp-pipedrive" becomes "mcppipedrive".
package mcppipedrive
...
// HardError is a tool-level error that should NOT be serialized as a
// CallToolResult — it becomes a protocol-level error instead. Use for missing
// client, missing config, or unrecoverable infra failures.
...
// MustTool wraps ConvertTool and panics on schema error so schema bugs surface
// at init rather than on the first call.
...
func ConvertTool[T any, R any](name, description string, toolHandler ToolHandlerFunc[T, R], options ...mcp.ToolOption) (mcp.Tool, server.ToolHandlerFunc, error) {
	...
	handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		config := pipedrive.ConfigFromContext(ctx)
		ctx, span := otel.Tracer("mcp-pipedrive").Start(ctx, fmt.Sprintf("mcp.tool.%s", name))
		defer span.End()
		span.SetAttributes(
		...
		if handlerErr != nil {
			...
			return &mcp.CallToolResult{
				Content: []mcp.Content{
					mcp.TextContent{Type: "text", Text: handlerErr.Error()},
				},
				IsError: true,
			}, nil
		}
